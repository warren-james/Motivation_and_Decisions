...
In these models, group membership, delta, and the interaction between these were added as parameters. With increasing separation between the boxes (delta), there should be a drop in performance. Delta was scaled by dividing the value (in pixels) by the maximum value at which each participant was tested. This was carried out for both Actual and Expected accuracy (see the main text for details).

Actual accuracy: As can be seen in the left plot below, participants in the control group had the lowest rate of success across all separations. For all groups, increased separation of the boxes did result in a lower accuracy rate. The effect of delta was larger for the Optimal and Motivated groups when compared to the Control group, and this is likely due to the lower intercept for the Control group.
